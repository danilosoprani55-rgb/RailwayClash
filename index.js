console.clear()

const figlet = require('figlet')
const net = require('net')
const MessageFactory = require('./Protocol/MessageFactory')
const server = new net.Server()
const Messages = new MessageFactory()
const config = require('./config.json')

const PORT = process.env.PORT || config.Server.Port || 3000

server.on('connection', async (client) => {
  client.setNoDelay(true)

  client.log = function (text) {
    if (config.Server.Debug) {
      if (config.Server.StreamerMode) {
        return console.log(`[*] >> ${text}`)
      } else {
        return console.log(`[${this.remoteAddress?.split(':').slice(-1)}] >> ${text}`)
      }
    } else {
      return console.log(`[*] >> ${text}`)
    }
  }

  client.log('A wild connection appeared!')

  const packets = Messages.getPackets()

  client.on('data', async (packet) => {
    try {
      const message = {
        id: packet.readUInt16BE(0),
        len: packet.readUIntBE(2, 3),
        version: packet.readUInt16BE(5),
        payload: packet.slice(7, 7 + packet.readUIntBE(2, 3)),
        client,
      }

      if (packets.includes(String(message.id))) {
        const PacketClass = Messages.handle(message.id)
        const packetInstance = new PacketClass(message.payload, client)

        if (config.Server.Debug) {
          client.log(`Gotcha ${message.id} (${packetInstance.constructor.name}) packet!`)
        }

        await packetInstance.decode()
        await packetInstance.process()
      } else {
        client.log(`Gotcha undefined ${message.id} packet!`)
      }
    } catch (e) {
      console.log('[DATA ERROR]', e)
    }
  })

  client.on('end', () => {
    client.log('Client disconnected.')
  })

  client.on('error', (error) => {
    try {
      client.log('A wild error!')
      console.log(error)
      client.destroy()
    } catch (e) {}
  })
})

console.log(figlet.textSync('RailwayClash', { font: 'Slant' }))

server.once('listening', () => {
  console.log(`[SERVER] >> RailwayClash started on port ${PORT}!`)
})

server.listen(PORT)

process.on("uncaughtException", e => console.log('[UNCAUGHT]', e))
process.on("unhandledRejection", e => console.log('[REJECTION]', e))
